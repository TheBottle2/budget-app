...
export default function HomeScreen({ navigation }) {
  ...
  const verileriYukle = async () => {
    try {
      ...
      const filtreliAy = hepsi.filter((t) => t.tarih.startsWith(`${yil}-${ayStr}`));
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.bosText}>Bu ay henüz işlem yok 🗒️</Text>}
      />
...
const styles = StyleSheet.create({
  ...
  header:           { backgroundColor: '#6C63FF', padding: 16, paddingTop: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  headerBaslik:     { color: '#fff', fontSize: 20, fontWeight: 'bold' },
  cikisBtn:         { color: '#fff', fontSize: 14, opacity: 0.8 },
  aySecici:         { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#6C63FF', paddingBottom: 16 },
  ayBtn:            { paddingHorizontal: 20 },
  ayBtnText:        { color: '#fff', fontSize: 28, fontWeight: '300' },
  ayMetin:          { color: '#fff', fontSize: 18, fontWeight: '600', minWidth: 150, textAlign: 'center' },
  liste:            { padding: 16 },
  listBaslikSatir:  { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 },
  listBaslik:       { fontSize: 16, fontWeight: 'bold', color: '#333' },
  tumunuGor:        { fontSize: 13, color: '#6C63FF', fontWeight: '600' },
  bosText:          { textAlign: 'center', color: '#aaa', marginTop: 40, fontSize: 15 },
  ekleBtn:          { backgroundColor: '#6C63FF', margin: 16, borderRadius: 14, padding: 16, alignItems: 'center' },
  ekleBtnText:      { color: '#fff', fontSize: 17, fontWeight: 'bold' },
});